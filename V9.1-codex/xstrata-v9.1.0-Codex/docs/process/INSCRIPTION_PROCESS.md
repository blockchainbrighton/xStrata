...
2. **Chunking:** Chunk size is read from `#chunk-size-select` (hidden by default).
   ...
   - v9.2.7 forces 16 KB chunks to keep 50-chunk batches under type limits.
   - v9.2.8 forces 16 KB chunks to keep 50-chunk batches under type limits.
...
1. **Wallet Prompt:** `begin-inscription` is submitted only for a fresh inscription.
   - v9.2.8 includes a fixed begin royalty (0.1 STX).
   - Miner fees are estimated by the wallet at signing; the UI shows fee-rate
     estimates only.
...
2. **Wallet Prompt:** `seal-inscription` is submitted.
   - v9.2.8 includes a seal royalty (0.1 STX base + 0.01 STX per chunk).
