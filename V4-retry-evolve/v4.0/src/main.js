...
const getStacksTx = async () => {
    if (!_stacksTx) _stacksTx = await import('@stacks/transactions');
    return _stacksTx;
};
...
async function checkPendingStatus(root) {
    ...
    const { bufferCV, standardPrincipalCV, cvToValue } = await getStacksTx();
...
async function handleSelectedFile(file) {
    if (!file) return;
    const { chunkFile, computeMerkleRoot, bufToHex } = await getMerkle();
...
async function startChunkUploads(hash, statusEl, specificIndices = null) {
    ...
    const { address, name } = getContractDetails();
    const { bufferCV, uintCV, PostConditionMode, AnchorMode } = await getStacksTx();
