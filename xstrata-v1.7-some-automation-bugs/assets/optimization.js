...
(function() {
    ...
    window.fetch = async function(url, options = {}) {
        ...
        // 1. Resolve URL string and check for Request object
        ...
        if (urlStr.includes('api.mainnet.hiro.so') || urlStr.includes('api.testnet.hiro.so')) {
            ...
            if (requestObj) {
                // Clone the request with new URL (Request constructor accepts another Request as init)
                url = new Request(urlStr, requestObj);
            } else {
                url = urlStr;
            }
        }

        // 3. Determine Method for Cache Key
        const method = (requestObj && requestObj.method) ? requestObj.method : (options.method || 'GET');
        const cacheKey = getCacheKey(urlStr, { method });
