...
const expected = [
  ...
  "CONTRACT_SOURCE_BATCHXR_V9_2_11",
...
const tests = [
  ...
  {
    name: "v9.2.11 stores token uris per id",
    run() {
      const text = fs.readFileSync(contractsPath, "utf8");
      const match = text.match(
        /CONTRACT_SOURCE_BATCHXR_V9_2_11\s*=\s*`([\s\S]*?)`/
      );
      assert(match, "Expected v9.2.11 contract source");
      const body = match[1] || "";
      assert(
        body.includes("(define-map TokenURIs") &&
          body.includes("(get-token-uri"),
        "Expected v9.2.11 to define TokenURIs map and get-token-uri"
      );
    }
  }
];
